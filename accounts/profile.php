...
if (isset($_POST['username'], $_POST['password'], $_POST['cpassword'], $_POST['email'])) {
	...
	if (empty($msg)) {
		...
		$stmt = $conn->prepare('SELECT COUNT(*) FROM accounts WHERE (username = ? OR email = ?) AND username != ? AND email != ?');
		...
		if ($result = $stmt->fetchColumn()) {
			...
			$stmt = $conn->prepare('UPDATE accounts SET username = ?, password = ?, email = ?, activation_code = ? WHERE id = ?');
